...
class ExamenController extends Controller
{
    ...
    // //Pour affectation des surveillants automatique
    // public function generateReservistList($date)
    // {
    //     // Récupérer les enseignants qui ne sont pas déjà assignés
    //     $enseignants = Enseignant::doesntHave('examens')
    //         ->whereDoesntHave('reserviste', function ($query) use ($date) {
    //             $query->where('date', $date);
    //         })
    //         ->get();

    //     // Créer une entrée pour chaque enseignant dans la liste des réservistes
    ...
    // public function assignReservistIfNeeded($date, $salleId, $examenId)
    // {
    //     // Récupérer le premier surveillant réserviste non affecté pour cette date
    //     $reservist = SurveillantReserviste::where('date', $date)
    //         ->where('affecte', false)
    //         ->first();

    //     if ($reservist) {
    //         // Affecter le surveillant réserviste
    //         DB::table('examen_salle_enseignant')->insert([
    //             'id_examen' => $examenId,
    //             'id_salle' => $salleId,
    //             'id_enseignant' => $reservist->enseignant_id,
    //             'created_at' => now(),
    //             'updated_at' => now(),
    //         ]);

    //         // Marquer le surveillant réserviste comme affecté
    //         $reservist->affecte = true;
    //         $reservist->save();
    //     }
    // }
    ...
    // /**
    //  * Assigne les surveillants aux salles pour un examen spécifique.
    //  */
    ...
    //         foreach ($examen->salles as $salle) {
    //             // Déterminer le nombre de surveillants nécessaires
    //             $nombreSurveillants = $salle->capacite >= 100 ? 4 : ($salle->capacite >= 80 ? 3 : 2);
    ...
    //                     ->where('examens.date', $date)
    //                     ->where(function ($query) use ($heure_debut, $heure_fin) {
    //                         $query->whereBetween('examens.heure_debut', [$heure_debut, $heure_fin])
    //                               ->orWhereBetween('examens.heure_fin', [$heure_debut, $heure_fin])
    //                               ->orWhere(function ($query) use ($heure_debut, $heure_fin) {
    //                                   $query->where('examens.heure_debut', '<=', $heure_debut)
    //                                         ->where('examens.heure_fin', '>=', $heure_fin);
    //                               });
    //                     })
    //                     ->exists();
    ...
    //             // Si le nombre requis de surveillants n'est pas atteint, essayer d'assigner des réservistes
    //             if ($surveillantsAffectes < $nombreSurveillants) {
    //                 $this->assignReservistIfNeeded($date, $salle->id, $examen->id, $nombreSurveillants - $surveillantsAffectes, $heure_debut, $heure_fin);
    //             }
    ...
    //         // Affecter les surveillants réservistes pour la demi-journée
    //         $this->assignReservistsForHalfDay($date, $heure_debut);
    ...
    //     } catch (\Exception $e) {
    //         DB::rollBack();
    //         return back()->withErrors(['error' => 'Une erreur est survenue lors de l\'assignation des surveillants.']);
    //     }
    // }
