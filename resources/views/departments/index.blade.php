<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center bg-green-100 p-4 rounded-lg shadow-md">
            ...
            <a href="{{ route('departments.create') }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition-colors duration-300 ease-in-out" onclick="return confirm('{{ __('Êtes-vous sûr de vouloir ajouter un nouveau département ?') }}');">
                            ...
                            @forelse ($departments as $department)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="{{ route('departments.edit', $department->id_department) }}" class="text-blue-600 dark:text-blue-400 hover:text-yellow-600 dark:hover:text-yellow-400 mr-2" title="{{ __('Modifier') }}" onclick="return confirm('{{ __('Êtes-vous sûr de vouloir modifier ce département ?') }}');">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline-block mr-1" viewBox="0 0 20 20" fill="currentColor">
